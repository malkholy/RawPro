...
export default function TreasuryPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await apiCall("Get Treasury Transactions", { LineData: JSON.stringify({}) })
      setAll(res.List0 || [])
  ...
  const del = async (id) => {
    ...
    try {
      await apiCall("Delete Treasury Transaction", { LineData: JSON.stringify({ TxID: id }) })
      load()
    } catch (e) { alert(e.message) }
  }
  ...
  return (
    <div>
      ...
      <SectionCard
        ...
        action={<Btn onClick={openAdd}>+ New {tab}</Btn>}
      >
        <Tabs
