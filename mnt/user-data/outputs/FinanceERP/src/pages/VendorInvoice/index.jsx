...
export default function VendorInvoicePage() {
  ...
  const openEdit = async (row) => {
    ...
    setModal(true)
    try {
      const res = await apiCall("Get Vendor Invoice Lines", { LineData: JSON.stringify({ InvoiceID: row.InvoiceID }) })
  ...
  const del = async (id) => {
    ...
    try {
      await apiCall("Delete Vendor Invoice", { LineData: JSON.stringify({ InvoiceID: id }) })
      load()
    } catch (e) { alert(e.message) }
  }
        ...
        <FormGrid>
          <Field label="Vendor Name">
            <Input value={form.VendorName} onChange={e => setForm({ ...form, VendorName: e.target.value })}
              placeholder="Vendor name" list="vendor-list" />
